    ...
    @foreach($categories as $category)
        ...
        @if ($category->parent_id == 0)
            ...
            @endif
                ...
                <td></td>
                <td>{{ $category->getCurrentCount($category->name) }}</td>
            </tr>
            @if(count($children) > 0)
                @foreach($children as $child)
                    <tr class="client_info cat{{ $category->id }} child choose_category" onclick='$("tr.products_details").hide();$("#selected_cat").val({{ $child->id }});$("tr#countries-{{ $child->id }}").slideToggle();'>
                        ...
                        <td></td>
                        <td></td>
                        <td class="td_center_text"><span class="client-table-dark">{{ $child->getCurrentCount($child->name) }}</span></td>
                    </tr>
                ...
                <div class="form-group">
                    ...
                    <select name="memory" id="memory" class="form-control">
                        @foreach($memories as $val)
                            <option value="{{$val->value}}">{{$val->value}} Gb</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label for="position">Порядок сортировки</label>
